...
public class MultipleForwardTileManager : MonoBehaviour
{
    ...
    private void AllChildsWhite()
    {
        int numChilds = transform.childCount;

        for (int i = 0; i < numChilds; ++i)
        {
            transform.GetChild(i).GetChild(3).GetComponent<Renderer>().material.SetColor("_Color", Color.white);
        }
    }
}
